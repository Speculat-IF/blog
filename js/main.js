...
if (window.location.href.includes("index.html") || window.location.href.includes("https://speculat-if.github.io/blog/")) {
    fetch(url)
        ...
        var img = new Image(); img.onload = () => console.log('No error detected'); 
        for (let i = 0; i < 3; i++) {
            let post = posts[i];
    ...
    fetch(url)
        ...
        .then((allArticle) => {
            ...
            for (let i = 1; i < dataLength; i++) {
                ...
                img.onerror = () => 
                window.href.location = "https://developer.mozilla.org/fr/docs/Web/HTTP/Status/418"; 
                img.src = imagePath;
                ...
                if (filter.innerHTML != "Tous") {
                    posts.forEach((post) => {
                        ...
                        if (tags != null) {
                            ...
                            if (tag2 != null) {
                                if (tag1.innerHTML.includes(filter.innerHTML) || tag2.innerHTML.includes(filter.innerHTML)) {
                                    post.style.display = "flex";
                                }
                            }
                        }
...
if (window.location.href.includes("article.html")) {
    fetch(url)
        ...
        textP1.innerHTML = searchedArticle.contenu.split(" ").slice(0,searchedArticle.contenu.split(" ").length/2).join(" ") + "...";
        let textP2 = document.querySelector(".textP2");
        textP2.innerHTML = "..." + searchedArticle.contenu.split(" ").slice(searchedArticle.contenu.split(" ").length/2,searchedArticle.contenu.split(" ").length).join(" ");
