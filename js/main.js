  ...
  fetch(url)
    ...
    .then((data) => {
      ...
      for (let i = Math.max(0, dataLength - 4); i < dataLength; i++) {
        ...
        var imagePath = "img/Barghest.png";
        var img = new Image();
        img.onload = () => console.log("Good is working");
        img.onerror = () =>
          (window.href.location =
            "https://developer.mozilla.org/fr/docs/Web/HTTP/Status/418");
        img.src = imagePath;
        resume.innerHTML =
          data.articles[i].contenu.split(" ").slice(0, 20).join(" ") +
          "..." +
          "<br><br>" +
          data.articles[i].date;
...
if (window.location.href.includes("article.html")) {
  ...
  fetch(url)
    ...
    .then((data) => {
      ...
      underTitre.innerHTML =
        searchedArticle.date + " - " + searchedArticle.auteur;
      firstdiv.appendChild(titre);
      firstdiv.appendChild(underTitre);

      let halftext = document.createElement("p");
      halftext.innerHTML = searchedArticle.contenu
        .split(" ")
      ...
      secondhalftext.innerHTML = searchedArticle.contenu
        .split(" ")
        .slice(
